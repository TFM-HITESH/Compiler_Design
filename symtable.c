...
void addSymbol(char *symbol, char *type, int size)
{
...
void printSymbolTable()
{
    ...
    {
        printf("%d\t\t%s\t%s\t%d\n", symbolTable[i].location, symbolTable[i].symbol,
...
{
    return strcmp(str, "for") == 0 || strcmp(str, "while") == 0 ||
           strcmp(str, "if") == 0 || strcmp(str, "else") == 0 ||
           strcmp(str, "return") == 0 || strcmp(str, "break") == 0 ||
           strcmp(str, "continue") == 0;
}
int getSizeOfType(const char *type)
{
    ...
    while (i < length)
    {
        ...
        if (isalpha(input[i]))
        {
            ...
            else
            {
                ...
                if (currentType[0] != '\0')
                {
                    int size = getSizeOfType(currentType);
                    if (i < length && input[i] == '[')
                    {
                        i++;
                        j = 0;
                        char sizeStr[MAX_TOKEN_SIZE];
                        ...
                        {
                            i++;
                            int arraySize = stringtoInt(sizeStr);
                            size *= arraySize;
                        }
                        addSymbol(token.value, strcat(currentType, "*"), size);
                    }
                    else
                    {
                        addSymbol(token.value, currentType, size);
                    }
                }
                else
                {
        ...
        {
            token.value[j++] = input[i++];
            if (i < length && isOperator(input[i]))
            {
                token.value[j++] = input[i++];
                token.value[j] = '\0';
